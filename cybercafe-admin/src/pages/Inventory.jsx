...
import { getInventory, addInventoryItem, updateInventoryItem, deleteInventoryItem, getInventorySettings, updateInventorySettings, sellInventoryItem, connectSocket, getTransactions, updateInventoryAccessControl, getPortalUsers, getAgentUsers, clearAllInventory, clearSalesHistory, initiateMpesaPush, checkMpesaStatus } from '../services/api';
...
function Inventory() {
    ...
    const completeSale = async (values, paymentMethod) => {
        try {
            const result = await sellInventoryItem(sellingItem._id, { quantity: values.quantity, reason: values.reason, paymentMethod: paymentMethod, clientId: 'admin' });
            setLastSaleResult({ itemName: sellingItem.name, quantity: values.quantity, unitPrice: sellingItem.price, totalAmount: sellingItem.price * values.quantity, previousStock: result.item?.previousStock, currentStock: result.item?.currentStock, seller: result.transaction?.seller || 'admin', reason: values.reason, paymentMethod: paymentMethod });
            message.success(`Sold ${values.quantity}x ${sellingItem.name}`); fetchInventory(); fetchSalesHistory();
        } catch (error) { message.error(error.response?.data?.error || 'Sale complete failed'); }
    };
    ...
    const submitSell = async (values) => {
        ...
        try {
            if (values.paymentMethod === 'mpesa') {
                const totalAmount = sellingItem.price * values.quantity;
                const pushRes = await initiateMpesaPush({
                    phoneNumber: values.phoneNumber,
                    amount: totalAmount,
                    accountReference: 'Inv-' + sellingItem.name.substring(0, 10),
                    transactionDesc: 'Sale: ' + sellingItem.name
                });
                
                if (pushRes.success) {
                    message.loading({ content: 'Waiting for M-Pesa PIN & payment confirmation...', key: 'mpesa-poll', duration: 0 });
                    let attempts = 0;
                    const poll = setInterval(async () => {
                        attempts++;
                        try {
                            const statusRes = await checkMpesaStatus(pushRes.checkoutRequestId);
                            if (statusRes.status === 'completed') {
                                clearInterval(poll);
                                message.success({ content: 'M-Pesa Payment Received!', key: 'mpesa-poll' });
                                completeSale(values, 'mpesa');
                            } else if (statusRes.status === 'failed') {
                                clearInterval(poll);
                                message.error({ content: 'M-Pesa Payment Failed: ' + statusRes.failureReason, key: 'mpesa-poll' });
                            } else if (attempts > 12) { // 1 min timeout
                                clearInterval(poll);
                                message.error({ content: 'M-Pesa Payment Timed Out', key: 'mpesa-poll' });
                            }
                        } catch (e) {
                    ...
                    message.error({ content: pushRes.error || pushRes.message || 'Failed to initiate M-Pesa push', key: 'mpesa-poll' });
                }
            } else {
                completeSale(values, 'cash');
            }
        } catch (error) { 
            message.destroy('mpesa-poll');
                    ...
                    <>
                        ...
                        <Form form={sellForm} layout="vertical" onFinish={submitSell}>
                            ...
                            <Form.Item noStyle shouldUpdate={(prev, cur) => prev.paymentMethod !== cur.paymentMethod}>
                                {() => {
                                    return sellForm.getFieldValue('paymentMethod') === 'mpesa' ? (
                                        <Form.Item name="phoneNumber" label="M-Pesa Phone Number" rules={[{ required: true, message: 'Please enter Safaricom number' }]}>
                                            <Input placeholder="07XX XXX XXX" prefix={<MobileOutlined />} />
                                        </Form.Item>
                                    ) : null;
                                }}
                            </Form.Item>
