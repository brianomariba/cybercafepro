...
async function startDataCollection() {
    ...
    try {
        startPrintDialogMonitor((data) => {
            ...
            if (data.finalized) {
                console.log(`[PRINT-DIALOG] FINALIZED: copies=${data.copies} color="${data.color}" media="${data.mediaType}" paper="${data.paperSize}" printer="${data.printer}" pages="${data.pages}" [${data.source}]`);
            }
        });
    } catch (e) {
    ...
    async function processGroupedJob(grouped) {
        ...
        // CRITICAL: UI Automation dialog data is the PRIMARY source for print settings.
        // EPSON drivers completely hide copies, color, and media type from ALL spooler APIs
        // (DEVMODE, WMI, PrintTicket, Event 805). The ONLY reliable source is reading the
        // actual print dialog UI before the user clicks Print.
        // Data is NEVER uploaded from the monitor itself - only when Event 307 confirms a print.
        const DIALOG_CACHE_MAX_AGE_MS = 120000; // 120s - generous window for slow EPSON printers
        const printerKey = (job.printer || '').toLowerCase().trim();
        let dialogData = printDialogCache.get(printerKey);
        let dialogMatchMethod = 'exact';
        
        if (!dialogData) {
            // FUZZY FALLBACK 1: Try partial string match (e.g. "epson l3250" vs "epson l3250 series")
            for (const [k, v] of printDialogCache.entries()) {
                if (k.length > 3 && (printerKey.includes(k) || k.includes(printerKey))) {
                    const age = Date.now() - (typeof v.timestamp === 'string' ? new Date(v.timestamp).getTime() : (v.timestamp || 0));
                    if (age < DIALOG_CACHE_MAX_AGE_MS) {
                        dialogData = v;
                        dialogMatchMethod = 'partial_name';
                        console.log(`[PRINT] UI-MATCH: Partial printer name "${k}" -> "${printerKey}"`);
                        break;
                    }
                }
            }
        ...
        if (!dialogData && printDialogCache.has('unknown')) {
            // FUZZY FALLBACK 2: Word backstage often hides the printer's UI name completely.
            const unknownData = printDialogCache.get('unknown');
            const unknownAge = Date.now() - (typeof unknownData.timestamp === 'string' ? new Date(unknownData.timestamp).getTime() : (unknownData.timestamp || 0));
            if (unknownAge < DIALOG_CACHE_MAX_AGE_MS) {
                dialogData = unknownData;
                dialogMatchMethod = 'unknown_key';
                console.log(`[PRINT] UI-MATCH: Matched "unknown" key to "${printerKey}" (${Math.round(unknownAge/1000)}s ago)`);
            }
        }

        if (!dialogData) {
            // FUZZY FALLBACK 3: Use ANY recent finalized dialog entry as last resort.
            // When only one printer is in the dialog cache, it almost certainly belongs to this job.
            let bestEntry = null;
            let bestAge = Infinity;
            for (const [k, v] of printDialogCache.entries()) {
                const age = Date.now() - (typeof v.timestamp === 'string' ? new Date(v.timestamp).getTime() : (v.timestamp || 0));
                if (age < DIALOG_CACHE_MAX_AGE_MS && v.finalized && age < bestAge) {
                    bestEntry = { key: k, data: v };
                    bestAge = age;
                }
            }
            if (bestEntry) {
                dialogData = bestEntry.data;
                dialogMatchMethod = 'most_recent_finalized';
                console.log(`[PRINT] UI-MATCH: Using most recent finalized dialog "${bestEntry.key}" for "${printerKey}" (${Math.round(bestAge/1000)}s ago)`);
            }
        }

        let dialogColorOverride = '';
        let dialogDataValid = false;
        if (dialogData) {
            const age = Date.now() - (typeof dialogData.timestamp === 'string' ? new Date(dialogData.timestamp).getTime() : (dialogData.timestamp || 0));
            if (age < DIALOG_CACHE_MAX_AGE_MS) {
                dialogDataValid = true;
                // UI Automation is the PRIMARY source for copies - always use it
                // EPSON drivers ALWAYS report copies=1 from spooler, so dialog value wins
                if (dialogData.copies && dialogData.copies >= 1) {
                    if (dialogData.copies !== copies) {
                        console.log('[PRINT] UI-PRIMARY: copies ' + copies + ' -> ' + dialogData.copies + ' (from ' + (dialogData.source || 'dialog') + ', ' + dialogMatchMethod + ', ' + Math.round(age/1000) + 's ago)');
                    }
                    copies = dialogData.copies;
                }
                // Use total sheets from dialog (browser: "5 sheets of paper", office: pages*copies)
                if (dialogData.totalSheets > 0 && dialogData.totalSheets > totalPages) {
                    console.log('[PRINT] UI-PRIMARY: totalPages ' + totalPages + ' -> ' + dialogData.totalSheets + ' (from dialog sheets count)');
                    totalPages = dialogData.totalSheets;
                }
                // Use pages from dialog if available and higher than spooler-reported
                if (dialogData.pages && parseInt(dialogData.pages) > totalPages) {
                    console.log('[PRINT] UI-PRIMARY: totalPages ' + totalPages + ' -> ' + parseInt(dialogData.pages) + ' (from dialog page count)');
                    totalPages = parseInt(dialogData.pages);
                }
                ...
                // Also clean up the matched key if different from printerKey
                if (dialogMatchMethod !== 'exact') {
                    for (const [k] of printDialogCache.entries()) {
                        if (k === dialogData.printer?.toLowerCase()?.trim() || k === 'unknown') {
                            printDialogCache.delete(k);
                        }
                    }
                }
                dataSource = 'ui_automation';
            } else {
                // Stale data - expired, discard it
                printDialogCache.delete(printerKey);
            }
        } else {
            console.log('[PRINT] UI-WARNING: No dialog data found for "' + printerKey + '" - using spooler defaults (copies may be wrong for EPSON)');
        }
