...
while (\$true) {
    ...
    foreach (\$p in \$officeProcs) {
        try {
            ...
            # Search for Copies control using MULTIPLE name variants and control types
            # Office versions use different labels: "Copies:", "Copies", "Number of copies"
            # Control types vary: Edit, Spinner, Custom
            \$copiesEdit = \$null
            \$copiesNames = @("Copies:", "Copies", "Number of copies", "Number of Copies:", "Number of copies:")
            \$copiesTypes = @([System.Windows.Automation.ControlType]::Edit,
            ...
            foreach (\$cName in \$copiesNames) {
                ...
                foreach (\$cType in \$copiesTypes) {
                    \$nc = New-Object System.Windows.Automation.PropertyCondition(
                        [System.Windows.Automation.AutomationElement]::NameProperty, \$cName)
            ...
            # FALLBACK: Scan all editable controls for anything with "cop" in the name
            if (-not \$copiesEdit) {
                try {
            ...
            if (\$copiesEdit) {
                ...
                try {
                    ...
                    # Try RangeValuePattern (some spinners use this)
                    try {
                        \$rvp = \$copiesEdit.GetCurrentPattern(
                            [System.Windows.Automation.RangeValuePattern]::Pattern)
                        \$copies = [int]\$rvp.Current.Value
                    } catch {}
                }
                if (\$copies -le 0) { \$copies = 1 }

                # Read printer name - try multiple label names used across Office versions
                \$printer = ""
                ...
                foreach (\$pLabel in \$printerLabels) {
                    ...
                    try {
                        ...
                        \$pEl = \$appWin.FindFirst(
                            [System.Windows.Automation.TreeScope]::Descendants, \$pc)
                        if (\$pEl) {
                            try {
                                \$pv = \$pEl.GetCurrentPattern(
                                    [System.Windows.Automation.ValuePattern]::Pattern)
                                \$printer = \$pv.Current.Value
                            } catch {
                                # Try SelectionPattern for combo boxes
                                try {
                                    \$sp = \$pEl.GetCurrentPattern(
                                        [System.Windows.Automation.SelectionPattern]::Pattern)
                                    \$sel = \$sp.Current.GetSelection()
                                    if (\$sel -and \$sel.Length -gt 0) {
                                        \$printer = \$sel[0].Current.Name
                                    }
                                } catch {}
                            }
                            # If still empty, try the element's Name if it looks like a printer
                            if (-not \$printer -and \$pEl.Current.Name -match '(?i)(epson|canon|hp|brother|xerox|ricoh|samsung|lexmark)') {
                                \$printer = \$pEl.Current.Name
                            }
                        }
                    } catch {}
                }
                # Last resort: scan for any element whose Name contains a known printer brand
                if (-not \$printer) {
                    try {
                        \$allEls2 = \$appWin.FindAll([System.Windows.Automation.TreeScope]::Descendants,
                            [System.Windows.Automation.Condition]::TrueCondition)
                ...
                \$duplex = ""
                try {
                    \$duplexLabels = @("Two-Sided Printing", "Print on Both Sides", "Duplex", "2-Sided Printing")
                    foreach (\$dLabel in \$duplexLabels) {
                        \$dc = New-Object System.Windows.Automation.PropertyCondition(
                            [System.Windows.Automation.AutomationElement]::NameProperty, \$dLabel)
                ...
                # Capture paper/media type from Office settings area
                \$officePaper = ""
                \$officeMedia = ""
                try {
                    \$officePaper = Find-UIValue \$appWin "Paper Size"
                    if (-not \$officePaper) { \$officePaper = Find-UIValue \$appWin "Page Size" }
                    if (-not \$officePaper) { \$officePaper = Find-UIValue \$appWin "Document Size" }
                    \$officeMedia = Find-UIValue \$appWin "Paper Type"
                    if (-not \$officeMedia) { \$officeMedia = Find-UIValue \$appWin "Media Type" }
                    if (-not \$officeMedia) { \$officeMedia = Find-UIValue \$appWin "Media type" }
                } catch {}

                # Capture page count from backstage (e.g. "5 of 10 Pages" or page range text)
                \$officePages = ""
                try {
                    \$officePages = Find-UIValue \$appWin "Pages:"
                    if (-not \$officePages) { \$officePages = Find-UIValue \$appWin "Pages" }
                    # Also look for page count text like "X of Y Pages"
                    if (-not \$officePages) {
                        \$pgText = Find-UIText \$appWin '\d+\s+(of|/)\s+\d+\s+[Pp]age'
                        if (\$pgText -and \$pgText -match '(\d+)\s+(of|/)\s+(\d+)') {
                            \$officePages = \$Matches[3]  # total page count
                        }
                    }
                } catch {}
                # Calculate total sheets for Office
                \$officeSheets = 0
                if (\$officePages -match '^\d+$') {
                    \$officeSheets = [int]\$officePages * [Math]::Max(1, \$copies)
                }
                ...
                \$result = @{
                    ...
                    orient = ""; duplex = \$duplex; sheets = \$officeSheets
...
public class W32POI {
    ...
    public static void MakeInvisible(IntPtr hwnd) {
        ...
        SetWindowLong(hwnd, -20, wl | 0x80000);
    ...
    try {
        ...
        if ($hHiddenDesk -ne [IntPtr]::Zero) {
            ...
            for ($i = 0; $i -lt 800; $i++) {
                ...
                $hwnd = [W32POI]::FindWindow('#32770', "$PrinterName Printing Preferences")
                if ($hwnd -eq [IntPtr]::Zero) { $hwnd = [W32POI]::FindWindow($null, "$PrinterName Printing Preferences") }
                if ($hwnd -eq [IntPtr]::Zero) { $hwnd = [W32POI]::FindWindow('#32770', "$PrinterName Properties") }
                if ($hwnd -eq [IntPtr]::Zero) { $hwnd = [W32POI]::FindWindow($null, "$PrinterName Properties") }
                if ($hwnd -ne [IntPtr]::Zero) { 
                    [void][W32POI]::MakeInvisible($hwnd)
                    [void][W32POI]::MoveToCorner($hwnd)
                    break 
                }
            }
                ...
                if ($poiLabel -ne [IntPtr]::Zero) {
                    ...
                    for ($attempt = 1; $attempt -le 3; $attempt++) {
                        ...
                        for ($w = 0; $w -lt 500; $w++) {
                            ...
                            if ($poiDlg -ne [IntPtr]::Zero) { 
                                [void][W32POI]::MakeInvisible($poiDlg)
                                break 
                            }
                        }
                        if ($poiDlg -ne [IntPtr]::Zero) { break }
